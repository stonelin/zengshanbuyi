...
- **已完成 150 条**（`src/data/cases_v2.json` 里有 150 条记录）。
- 已完成的 ID：`case_002` 到 `case_141`（不含 `case_001`、`case_119`，
...
- **`case_091`~`case_141`**（"增删黄金策千金赋章"，全书理论密度最高、
  ...
  而不是简单套用 `resolveCaseBoard(raw)`，接手前建议先读一遍本文档
  第 4 节和下面这些具体案例的 `disputed.note`：`case_086` `case_088`
  `case_118` `case_124` `case_128`（这条比较特殊，两次起卦的应验对象
  发生了错位，不是常规的"取第二次起卦"）。
- 下一步从 `case_142` 开始（"天时章"，25 条案例），再往后是"身命章"
  （0 条，纯理论，跳过）、"终身财福章"（15 条）等一系列中小型章节，
  详见 `src/data/chapters.json` 里 `ch_043` 以后的 `case_ids`。
...
1. 从 `case_142` 继续往后做，一直到 `case_323`（`src/data/cases.json`
