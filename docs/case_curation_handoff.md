...
- **已完成 245 条**（`src/data/cases_v2.json` 里有 245 条记录）。
- 已完成的 ID：`case_002` 到 `case_212`（不含 `case_001`、`case_119`，
  两者理由见下方"已知缺口"），加上从边界识别问题里顺手捞回来的
  "bonus"案例：`case_019b` `case_020b` `case_020c` `case_036b` `case_040b`
  `case_047` `case_048` `case_049` `case_050` `case_064b` `case_077b`
  `case_086b` `case_090b` `case_111b` `case_132b`
  以及 `case_142`~`case_188` 区间新增的一大批（详见下方 "天时章" 说明，
  完整清单见 `scripts/test_engine.js` 的 `MANUAL_BOARD_CASE_IDS`）。
...
1. 从 `case_213` 继续往后做，一直到 `case_323`（`src/data/cases.json`
