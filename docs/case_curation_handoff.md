...
- **已完成 221 条**（`src/data/cases_v2.json` 里有 221 条记录）。
- 已完成的 ID：`case_002` 到 `case_188`（不含 `case_001`、`case_119`，
...
- **`case_167`~`case_181`**（"终身财福章"，15 条）、
  **`case_182`~`case_188`**（"终身功名有无章"，7 条，含 4 条 bonus：
  ...
  取错卦，已改手工构造）。
- 下一步从 `case_189` 开始（"寿元章"，8 条案例），再往后是"趋避章"
  （11 条）、"父母寿元章"（5 条）等一系列中小型章节，详见
  `src/data/chapters.json` 里 `ch_047` 以后的 `case_ids`。
...
1. 从 `case_189` 继续往后做，一直到 `case_323`（`src/data/cases.json`
